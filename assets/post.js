...
(function () {
  ...
  function fixLinks(html) {
    // Prevent double "post/post/" — only add "post/" if not already present
    return html.replace(/href="([^"]*\.html)"/g, function(match, slug) {
      // If it already starts with "post/", leave it alone
      // Otherwise, add "post/" prefix
  ...
  function renderSidebarAndRelated(post) {
    ...
    if (moreList) {
      var moreHTML = more.map(BuildWire.sidebarItemHTML).join("");
      moreList.innerHTML = fixLinks(moreHTML);
    }
    ...
    if (relatedGrid) {
      if (related.length) {
        var relatedHTML = related.map(BuildWire.cardHTML).join("");
        relatedGrid.innerHTML = fixLinks(relatedHTML);
